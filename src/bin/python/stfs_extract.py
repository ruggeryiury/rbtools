...
def stfs_extract_on_root(stfs_file_path: str, dest_path: str) -> str:
  dest = Path(dest_path)
...
if __name__ == '__main__':
  parser = argparse.ArgumentParser(description='Xbox 360 STFS File Extractor')
  parser.add_argument('stfs_file_path', help='The STFS file you want to extract its contents', type=str)
  parser.add_argument('dest_path', help='The folder path where you want the files to be extracted to', type=str)
  parser.add_argument('--extract-on-root', help='Extract all files on the root rather than recreate the entire STFS file system recursively', action=argparse.BooleanOptionalAction, default=False)
