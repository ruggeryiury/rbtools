...
def gen_key_inner(
    xbox: bool, hvkey: bytes, mogg_data: bytes, version: int
) -> bytearray:
    key_mask = bytearray(16)
    bad_mask_1 = bytearray(
        b"\xc3\xc3\xc3\xc3\x00\x01\x02\x03\x04\x05\x06\x07\x08\x09\x0a\x0b"
    )  # only if v13
    bad_mask_2 = bytearray(
        b"\x6c\x6c\x65\x63\x74\x69\x76\x65\x2d\x74\x6f\x6f\x6c\x73\x2d\x62"
    )  # only if v12
    hmx_header_size = int.from_bytes(mogg_data[16:20], "little")
    if xbox:
        key_mask[0:16] = mogg_data[
            20 + hmx_header_size * 8 + 16 + 32 : 20 + hmx_header_size * 8 + 16 + 48
        ]
    else:
        key_mask[0:16] = mogg_data[
            20 + hmx_header_size * 8 + 16 + 16 : 20 + hmx_header_size * 8 + 16 + 32
        ]
    ...
    magic_a = int.from_bytes(
        mogg_data[20 + hmx_header_size * 8 + 16 : 20 + hmx_header_size * 8 + 16 + 4],
        "little",
    )
    magic_b = int.from_bytes(
        mogg_data[
            20 + hmx_header_size * 8 + 16 + 8 : 20 + hmx_header_size * 8 + 16 + 12
        ],
        "little",
    )
...
def quickshuffle(key: bytearray) -> bytearray:
...
def supershuffle(key: bytearray) -> bytearray:
    key = shuffle1(key)
    key = shuffle2(key)
    key = shuffle3(key)
    key = shuffle4(key)
...
def shuffle1(key: bytearray) -> bytearray:
    for i in range(0, 8):
        o = roll(i << 2)
...
def shuffle2(key: bytearray) -> bytearray:
    for i in range(0, 8):
...
def shuffle3(key: bytearray) -> bytearray:
    for i in range(0, 8):
        o = roll(((7 - i) * 4) + 1)
        key[o], key[(i * 4) + 2] = key[(i * 4) + 2], key[o]
        key[(7 - i) * 4], key[(i * 4) + 3] = key[(i * 4) + 3], key[(7 - i) * 4]
...
def shuffle4(key: bytearray) -> bytearray:
    for i in range(0, 8):
        key[((7 - i) * 4) + 1], key[(i * 4) + 2] = (
            key[(i * 4) + 2],
            key[((7 - i) * 4) + 1],
...
def shuffle5(key: bytearray) -> bytearray:
    for i in range(0, 8):
        o = roll((i * 4) + 2)
...
def shuffle6(key: bytearray) -> bytearray:
    for i in range(0, 8):
        key[((7 - i) * 4) + 1], key[(i * 4) + 2] = (
            key[(i * 4) + 2],
            key[((7 - i) * 4) + 1],
...
def mash(key: bytearray, masher: bytes) -> bytearray:
    for i in range(0, 32):
...
def ascii_digit_to_hex(h: int) -> int:
    if h < 0x61 or 0x66 < h:
        if h < 0x41 or 0x46 < h:
            h = h - 0x30
            if h < 0:
...
def hex_string_to_bytes(s: bytearray) -> bytearray:
    arr = bytearray(16)
    for i in range(0, 16):
...
def o_funcs(a1: int, a2: int, op: int) -> int:
    match op:
...
def hmxa_to_ogg(decmogg_data: bytearray, ogg_offset: int, hmx_header_size: int) -> None:
    magic_a = int.from_bytes(
        decmogg_data[20 + hmx_header_size * 8 + 16 : 20 + hmx_header_size * 8 + 20],
        "little",
    )
    magic_b = int.from_bytes(
        decmogg_data[
...
def ogg_to_hmxa(encmogg_data: bytearray, ogg_offset: int, hmx_header_size: int) -> None:
    magic_a = int.from_bytes(
        encmogg_data[20 + hmx_header_size * 8 + 16 : 20 + hmx_header_size * 8 + 20],
        "little",
    )
    magic_b = int.from_bytes(
        encmogg_data[
...
def decrypt_mogg(
    xbox: bool, red: bool, fin: BufferedReader, fout: BufferedReader
) -> bool:
    failed = False
    mogg_data = fin.read()
    ...
    if version == 13 and decmogg_data[
        20 + hmx_header_size * 8 + 16 + 16 : 20 + hmx_header_size * 8 + 16 + 32
    ] == bytearray(b"\xc3\xc3\xc3\xc3\x00\x01\x02\x03\x04\x05\x06\x07\x08\x09\x0a\x0b"):
        decmogg_data[
            20 + hmx_header_size * 8 + 16 + 16 : 20 + hmx_header_size * 8 + 16 + 32
        ] = bytearray(
            b"\xa5\xce\xfd\x06\x11\x93\x23\x21\xf8\x87\x85\xea\x95\xe4\x94\xd4"
    ...
    if version == 12 and decmogg_data[
        20 + hmx_header_size * 8 + 16 + 16 : 20 + hmx_header_size * 8 + 16 + 32
    ] == bytearray(b"\x6c\x6c\x65\x63\x74\x69\x76\x65\x2d\x74\x6f\x6f\x6c\x73\x2d\x62"):
        decmogg_data[
            20 + hmx_header_size * 8 + 16 + 16 : 20 + hmx_header_size * 8 + 16 + 32
        ] = bytearray(
            b"\xf1\xb4\xb8\xb0\x48\xaf\xcb\x9b\x4b\x53\xe0\x56\x64\x57\x68\x39"
...
def reencrypt_mogg(
    xbox: bool, red: bool, enc_ver: int, fin: BufferedReader, fout: BufferedReader
) -> bool:
    failed = False
    ...
    if enc_ver == 13 and encmogg_data[
        20 + hmx_header_size * 8 + 16 + 16 : 20 + hmx_header_size * 8 + 16 + 32
    ] == bytearray(b"\xc3\xc3\xc3\xc3\x00\x01\x02\x03\x04\x05\x06\x07\x08\x09\x0a\x0b"):
        encmogg_data[
            20 + hmx_header_size * 8 + 16 + 16 : 20 + hmx_header_size * 8 + 16 + 32
