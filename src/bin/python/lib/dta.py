...
def clean_dta(dta_path: Union[str, PathLike[str]]) -> List[str]:
...
def parse(
    program: List[str],
) -> Union[List[List | int | float | str] | int | float | str]:
...
def read_from_tokens(
    ...
    token = tokens.pop(0)
    if token == "(":
        L: List[Union[List | int | float | str]] = []
        while tokens[0] != ")":
...
def atom(token: str) -> Union[int | float | str]:
    ...
    for s in range(len(parsed_name)):
        if s > 0:
            if type(parsed_name[s]) == list:
                new_song_name_list.append(
                    "(" + " ".join(str(x) for x in parsed_name[s]) + ")"
                )
            elif parsed_name[s] != "":
        ...
        if type(info) == list:
            if "drum_" in info[0]:
                ...
                for instr_tracks in info[1]:
                    song_info_dict[info[0]][instr_tracks[0]] = (
                        instr_tracks[1]
                        if type(instr_tracks[1]) == list
                        else [instr_tracks[1]]
                    )
...
def dict_from_parsed(
    parsed: Union[List[List | int | float | str] | int | float | str],
) -> Dict:
    big_songs_dict = {}
...
def parse_dta(dta_path: Union[str, PathLike[str]]) -> Dict:
