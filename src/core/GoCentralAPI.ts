...
export class GoCentralAPI {
   ...
   */
  static async getScores(songID: number, instrument: ScoreDataInstrumentTypes = 'band'): Promise<GoCentralLeaderboardResultObject> {
    ...
    if (res.data.leaderboard === null)
      ...
      return {
        ...
        scores: res.data.leaderboard.map<GoCentralLeaderboardObject>((score) => {
          ...
          return { name, difficulty: score.diff_id, score: score.score, scorePercent: score.notes_pct, stars: score.stars, platform }
        }),
      }
  }
}
